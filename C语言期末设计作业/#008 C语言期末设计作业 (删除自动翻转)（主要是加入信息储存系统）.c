...
void checkAndResumeUnfinishedGame(void)
{
    ...
    printf("已走步数：%d\n", storedMoveCnt);
    if (lastResult[0] != '\0') printf("最后结果（若有）：%s\n", lastResult);
    printf("请选择：\n");
    printf("  1 - 继续此对局\n");
    ...
    if (choice == 1)
    {
        ...
        else
        {
            ...
            fp = fopen(filename, "a");
            if (fp) { fprintf(fp, "[RESULT] 恢复失败异常\n[GAME_END]\n\n"); fclose(fp); }
        }
    }
    else if (choice == 2)
    {
...
void continueGame(void)
{
    ...
    while (1)
    {
        ...
        // 坐标转换：将用户看到的坐标转换为内部棋盘坐标（内部始终白方在下）
        int sr = raw_sr, sc = raw_sc, dr = raw_dr, dc = raw_dc;
        if (viewMode == 2) {  // 黑方在下模式，用户看到的是倒置的棋盘
            sr = 7 - sr;
            dr = 7 - dr;
        }
        // viewMode == 1 时不需要转换
