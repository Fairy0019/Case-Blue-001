...
int executeMove(int sr, int sc, int dr, int dc, int white) {
    ...
    if (tolower(piece) == 'p' && (dr == 0 || dr == 7)) {
        ...
        while (1) {
            ...
            if (promote == 'Q' || promote == 'R' || promote == 'N' || promote == 'B')
                break;
            printf("无效输入，请输入 Q, R, N, B : ");
        }
        board[dr][dc] = white ? promote : tolower(promote);
        while (getchar() != '\n'); // 清空输入缓冲区
    }
...
void playGame()
{
    ...
    while (1)
    {
        ...
        char dstFile = 'a' + dc;
        char dstRank = '8' - dr;
        sprintf(moveStr, "%c%c%c%c", srcFile, srcRank, dstFile, dstRank);
