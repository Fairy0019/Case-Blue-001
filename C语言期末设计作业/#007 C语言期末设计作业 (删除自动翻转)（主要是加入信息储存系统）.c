...
int viewMode = 1; // 1=始终白方在下，2=始终黑方在下（去掉自动模式）
...
void continueGame(void)
{
    ...
    while (1)
    {
        ...
        // 根据 viewMode 将用户看到的坐标转换为内部坐标（内部始终白方在下）
        int sr = raw_sr, sc = raw_sc, dr = raw_dr, dc = raw_dc;
        if (viewMode == 2)
        { // 黑方在下模式：用户看到的是倒置的棋盘，需要翻转行号
            sr = 7 - sr;
            dr = 7 - dr;
        }
        // viewMode == 1 时不需要转换，因为用户看到的就是白方在下
