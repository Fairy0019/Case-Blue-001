...
void checkAndResumeUnfinishedGame(void)
{
    ...
    // 如果未完成对局超过3个，只保留最后3个，前面的自动标记为中途退出（追加到文件末尾）
    ...
    {
        int nextWhite = (ug->moveCount % 2 == 0);
        const char *quitResult = nextWhite ? "白方中途退出" : "黑方中途退出";
...
void manageUnfinishedGames(void)
{
    ...
    while (1)
    {
        ...
        if (action == 1)
        {
            if (loadGameFromMoves((const char **)ug->moves, ug->moveCount))
            {
                ...
                for (int i = 0; i < moveCount; i++)
                    strcpy(currentGameMoves[i], ug->moves[i]);
                whiteToMove = (moveCount % 2 == 0);
                continueGame(); // 进入游戏，结束后会返回主菜单
                // 游戏结束后，需要重新加载未完成对局列表（因为可能增加了新的未完成对局）
                // 简单做法：重新扫描并填充 games 数组
                // 这里直接跳出函数，让用户下次再查看
                printf("游戏已结束，返回主菜单。\n");
                return;
            }
            else
            {
                ...
                // 从列表中移除该对局（简单做法：重新扫描一次，但为了避免复杂，直接 break 让用户重新进入菜单）
        ...
        else if (action == 2)
        {
            ...
            // 从列表中删除该对局（重新扫描）
            // 简化：刷新列表，重新扫描文件
            // 重新打开文件扫描并更新 games 数组和 unfinishedCount
            // 为了简洁，这里重新调用自身（递归），但要注意避免无限循环
            // 更好的做法：重新扫描并更新 games 数组
            // 由于时间，我们直接返回让用户重新选择菜单
