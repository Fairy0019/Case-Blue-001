...
// 更新战绩（通用）
void updateStats(const char *result) {
    char fname[64];
    sprintf(fname, "stats_%s.txt", currentUser.username);
    int win = 0, loss = 0, draw = 0, quit = 0;
    FILE *f = fopen(fname, "r");
    if (f) {
        fscanf(f, "赢:%d 输:%d 平:%d 中途退出:%d", &win, &loss, &draw, &quit);
        fclose(f);
    }
    if (strcmp(result, "白方赢") == 0) win++;
    else if (strcmp(result, "黑方赢") == 0) loss++;
    else if (strcmp(result, "平局") == 0) draw++;
    else if (strstr(result, "中途退出")) quit++;
    f = fopen(fname, "w");
    if (f) {
...
void registerUser() {
    ...
    printf("密码: ");
    fgets(p1, 20, stdin); trimNL(p1);
    printf("确认密码: ");
    fgets(p2, 20, stdin); trimNL(p2);
    if (strcmp(p1, p2)) {
        printf("密码不一致！\n");
        return;
    }
...
void changePassword() {
    ...
    printf("\n===== 修改密码 =====\n原密码: ");
    fgets(old, 20, stdin); trimNL(old);
    if (strcmp(old, currentUser.password)) {
        printf("原密码错误！\n");
        return;
    }
    printf("新密码: ");
    fgets(new1, 20, stdin); trimNL(new1);
    printf("确认新密码: ");
    fgets(new2, 20, stdin); trimNL(new2);
    if (strcmp(new1, new2)) {
        printf("两次新密码不一致！\n");
        return;
    }
    ...
    if (updated) {
        ...
        if (rename("users.tmp", "users.txt") != 0) {
            printf("修改失败，文件错误！\n");
            rename("users.tmp", "users.txt.bak");
        } else printf("密码修改成功！\n");
    } else {
        remove("users.tmp");
        printf("修改失败！\n");
...
// 比较两个未完成对局的棋步
int isSameGame(UnfinishedGame *a, UnfinishedGame *b) {
    if (a->moveCount != b->moveCount) return 0;
    for (int i = 0; i < a->moveCount; i++)
        if (strcmp(a->moves[i], b->moves[i]) != 0) return 0;
    return 1;
}

// 删除指定未完成对局（基于序号），并记录中途退出
void removeUnfinishedGameAtIndex(int idx) {
    UnfinishedGame games[10];
    int cnt = scanUnfinishedGames(games);
    if (idx < 0 || idx >= cnt) return;
    UnfinishedGame *target = &games[idx];
    int targetMoveCount = target->moveCount;
    const char *result = (targetMoveCount % 2 == 0) ? "白方中途退出" : "黑方中途退出";
    ...
    char line[256];
    int inGame = 0, skipThis = 0;
    UnfinishedGame curGame;
    curGame.moveCount = 0;
    int parseMoves = 0;
    ...
    while (fgets(line, sizeof(line), fp)) {
        trimNL(line);
        if (strcmp(line, "[GAME_START]") == 0) {
            if (inGame && curGame.moveCount > 0 && !skipThis) {
                fputs("[GAME_START]\n", tmp);
                for (int i = 0; i < curGame.moveCount; i++)
                    fprintf(tmp, "%d. %s\n", i + 1, curGame.moves[i]);
            }
            inGame = 1;
            curGame.moveCount = 0;
            ...
            if (!skipThis) {
                ...
                fprintf(tmp, "[RESULT] %s\n[GAME_END]\n\n", result);
                updateStats(result);
            }
            inGame = 0;
            parseMoves = 0;
            continue;
        } else if (inGame && parseMoves && line[0] >= '0' && line[0] <= '9') {
            char move[10];
            if (sscanf(line, "%*d. %s", move) == 1 && curGame.moveCount < MAX_MOVES) {
                strcpy(curGame.moves[curGame.moveCount], move);
                curGame.moveCount++;
                if (!skipThis && isSameGame(&curGame, target))
                    skipThis = 1;
            }
        } else if (!inGame && !parseMoves) {
            fputs(line, tmp);
            fputc('\n', tmp);
        }
    }
    // 处理末尾未结束的对局
    if (inGame && curGame.moveCount > 0) {
        if (!skipThis) {
            fputs("[GAME_START]\n", tmp);
            for (int i = 0; i < curGame.moveCount; i++)
                fprintf(tmp, "%d. %s\n", i + 1, curGame.moves[i]);
...
// 改进后的执行移动，promotePiece: 0表示交互询问，非0表示直接使用该升变棋子
int executeMove(int sr, int sc, int dr, int dc, int white, char promotePiece) {
                ...
                for (int dr = 0; dr < 8; dr++)
                    for (int dc = 0; dc < 8; dc++)
                        if (canMovePiece(sr, sc, dr, dc, white)) {
                            if (executeMove(sr, sc, dr, dc, white, 0)) {
                                game = bak;
                                return 1;
                            }
                            game = bak;
...
int loadGameFromMoves(char moves[][10], int moveCnt) {
    ...
    for (int i = 0; i < moveCnt; i++) {
        ...
        if (!executeMove(sr, sc, dr, dc, (i % 2 == 0), promo)) return 0;
    }
    return 1;
}
...
void replayGame(int idx) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (strstr(line, "[GAME_START]")) {
            cur++;
            if (cur == idx) { in = 1; moveCnt = 0; result[0] = 0; }
        } else if (in) {
    ...
    for (int i = 0; i < moveCnt; i++) {
        ...
        executeMove(sr, sc, dr, dc, (i % 2 == 0), promo);
        printBoard();
        printf("第%d步: %s\n", i+1, moves[i]);
...
void continueGame() {
    ...
    while (1) {
        ...
        char src[8] = "", dst[8] = "";
        char promo = 0;
        // 解析输入：支持 "e2 e4", "e2e4", "e7e8Q", "e7 e8Q"
        if (sscanf(line, "%7s %7s", src, dst) >= 1) {
            // 如果第二个词为空，尝试将第一个词拆成 src+dst
            if (dst[0] == 0 && strlen(src) >= 4) {
                int len = strlen(src);
                if (len == 4) {
        ...
        if (strcmp(src, "quit") == 0 || strcmp(src, "exit") == 0) {
            ...
            if (game.moveCount > 0) {
                ...
                // 没有走棋，删除孤立的 [GAME_START]
                char fname[64];
                sprintf(fname, "games_%s.txt", currentUser.username);
                FILE *fp = fopen(fname, "r");
                if (fp) {
                    FILE *tmp = fopen("games.tmp", "w");
                    char tmpLine[256];
                    ...
                    while (fgets(tmpLine, sizeof(tmpLine), fp)) {
                        if (firstStart && strstr(tmpLine, "[GAME_START]")) {
                            firstStart = 0;
                            continue;
                        }
                        fputs(tmpLine, tmp);
        ...
        char movingPiece = game.board[rs][cs];
        if (!executeMove(rs, cs, rd, cd, game.whiteToMove, promo)) {
            printf("非法移动！\n");
            continue;
        }
        // 生成代数记谱
        ...
        // 如果是升变且未指定promo（交互询问时），需要补上实际升变棋子
        if (tolower(movingPiece) == 'p' && (rd == 0 || rd == 7)) {
            char promoPiece = game.board[rd][cd];
            char tmp[10];
            sprintf(tmp, "%s%c", moveStr, promoPiece);
            strcpy(moveStr, tmp);
        }
        ...
        // 检查结果
        int inCheck = isKingInCheck(!game.whiteToMove);
        int hasMove = hasLegalMove(!game.whiteToMove);
        if (inCheck && !hasMove) {
            printBoard();
            ...
            printf("僵局，平局。\n");
            FILE *fp = openUserFile("games", "a");
            if (fp) { fprintf(fp, "[RESULT] 平局\n[GAME_END]\n\n", fp); fclose(fp); }
            updateStats("平局");
            return;
        }
        if (inCheck) printf("将军！\n");
...
void newGame() {
    ...
    while (1) {
        ...
        if (opt == 1) {
            ...
            cnt = scanUnfinishedGames(games); // 重新扫描
            if (cnt < 3) break;
        } else {
            printf("已取消开始新游戏。\n");
            return;
...
void listGames() {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        else if (strstr(line, "[GAME_END]") && in) {
            printf("%d. 结果: %s\n", gnum++, res);
            in = 0;
        }
    }
