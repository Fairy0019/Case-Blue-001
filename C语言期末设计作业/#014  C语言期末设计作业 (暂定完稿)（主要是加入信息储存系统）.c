...
void finishGame(const char *result, int win, int loss, int draw, int quit) {
    ...
    if (f) { fscanf(f, "赢:%d 输:%d 平:%d 中途退出:%d", &ow, &ol, &od, &oq); fclose(f); }
    ow += win; ol += loss; od += draw; oq += quit;
    f = fopen(fname, "w");
    if (f) { fprintf(f, "赢:%d 输:%d 平:%d 中途退出:%d\n", ow, ol, od, oq); fclose(f); }
}
...
void registerUser() {
    ...
    printf("\n===== 注册 =====\n用户名: "); fgets(u,20,stdin); trimNL(u);
    if (userExists(u)) { printf("用户名已存在！\n"); return; }
    printf("密码: "); fgets(p1,20,stdin); trimNL(p1);
    printf("确认密码: "); fgets(p2,20,stdin); trimNL(p2);
    if (strcmp(p1, p2)) { printf("密码不一致！\n"); return; }
    FILE *fp = fopen("users.txt", "a");
    if (fp) { fprintf(fp, "%s %s\n", u, p1); fclose(fp); printf("注册成功！\n"); }
    else printf("无法保存用户！\n");
}
...
int login() {
    char u[20], p[20];
    printf("\n===== 登录 =====\n用户名: "); fgets(u,20,stdin); trimNL(u);
    printf("密码: "); fgets(p,20,stdin); trimNL(p);
    FILE *fp = fopen("users.txt", "r");
...
void changePassword() {
    if (!currentUser.username[0]) { printf("请先登录！\n"); return; }
    char old[20], new1[20], new2[20];
    printf("\n===== 修改密码 =====\n原密码: "); fgets(old,20,stdin); trimNL(old);
    if (strcmp(old, currentUser.password)) { printf("原密码错误！\n"); return; }
    printf("新密码: "); fgets(new1,20,stdin); trimNL(new1);
    printf("确认新密码: "); fgets(new2,20,stdin); trimNL(new2);
    if (strcmp(new1, new2)) { printf("两次新密码不一致！\n"); return; }
    FILE *fp = fopen("users.txt", "r");
    FILE *tmp = fopen("users.tmp", "w");
    if (!fp || !tmp) { printf("系统错误！\n"); if(fp)fclose(fp); if(tmp)fclose(tmp); return; }
    char line[100], un[20], pw[20];
    int updated = 0;
    while (fgets(line, sizeof(line), fp)) {
        sscanf(line, "%s %s", un, pw);
        if (strcmp(un, currentUser.username) == 0) {
            fprintf(tmp, "%s %s\n", currentUser.username, new1);
            strcpy(currentUser.password, new1);
            updated = 1;
        } else fputs(line, tmp);
    }
    fclose(fp); fclose(tmp);
    remove("users.txt"); rename("users.tmp", "users.txt");
    printf(updated ? "密码修改成功！\n" : "修改失败！\n");
}

void logout() {
    if (!currentUser.username[0]) printf("尚未登录。\n");
    else { printf("用户 %s 已退出。\n", currentUser.username); currentUser.username[0] = 0; }
}

void showStats() {
    if (!currentUser.username[0]) { printf("请先登录！\n"); return; }
    char fname[64]; sprintf(fname, "stats_%s.txt", currentUser.username);
...
int scanUnfinishedGames(UnfinishedGame *games) {
    ...
    if (!fp) return 0;
    char line[256];
    int inGame = 0, count = 0, curMoveCnt = 0;
    char curMoves[MAX_MOVES][10];
    while (fgets(line, sizeof(line), fp)) {
        trimNL(line);
        if (strcmp(line, "[GAME_START]") == 0) {
            if (inGame && curMoveCnt > 0 && count < 10) {
                UnfinishedGame *ug = &games[count++];
                ug->moveCount = curMoveCnt;
                for (int i = 0; i < curMoveCnt; i++) strcpy(ug->moves[i], curMoves[i]);
            }
            inGame = 1; curMoveCnt = 0;
...
void markGameAsQuit(int moveCount) {
    const char *res = (moveCount % 2 == 0) ? "白方中途退出" : "黑方中途退出";
    FILE *fp = openUserFile("games", "a");
    if (fp) { fprintf(fp, "[RESULT] %s\n[GAME_END]\n\n", res); fclose(fp); }
    finishGame(res, 0,0,0,1);
}

int loadGameFromMoves(const char *moves[], int moveCnt); // forward
...
void checkAndResumeUnfinishedGame() {
    ...
    if (cnt > 3) {
        ...
        for (int i = 0; i < cnt-3; i++) markGameAsQuit(games[i].moveCount);
        for (int i = 0; i < 3; i++) games[i] = games[cnt-3+i];
        cnt = 3;
    }
    printf("\n========== 恢复系统 ==========\n发现 %d 个未完成对局：\n", cnt);
    for (int i = 0; i < cnt; i++) printf("%d. 已走 %d 步\n", i+1, games[i].moveCount);
    printf("选择对局 (1~%d) 或 0 退出: ", cnt);
    char buf[10]; fgets(buf,10,stdin);
    int ch = atoi(buf);
    if (ch<1 || ch>cnt) { printf("退出恢复。\n"); return; }
...
void manageUnfinishedGames() {
    UnfinishedGame games[10];
    int cnt = scanUnfinishedGames(games);
    if (cnt == 0) { printf("没有未完成对局。\n"); return; }
    while (1) {
        printf("\n未完成对局列表：\n");
        for (int i=0; i<cnt; i++) printf("%d. 步数 %d\n", i+1, games[i].moveCount);
        printf("选择 (1~%d) 或 0 返回: ", cnt);
        char buf[10]; fgets(buf,10,stdin);
        int ch = atoi(buf);
        if (ch == 0) break;
        if (ch<1 || ch>cnt) { printf("无效输入。\n"); continue; }
        UnfinishedGame *ug = &games[ch-1];
        printf("1-继续  2-放弃并记录退出  3-忽略 : ");
        fgets(buf,10,stdin);
        int act = atoi(buf);
...
void listGames() {
    ...
    printf("\n=== 历史对局 ===\n");
    while (fgets(line, sizeof(line), fp)) {
        if (strstr(line, "[GAME_START]")) { in=1; res[0]=0; }
        else if (strstr(line, "[RESULT]") && in) sscanf(line, "[RESULT] %[^\n]", res);
        else if (strstr(line, "[GAME_END]") && in) { printf("%d. 结果: %s\n", gnum++, res); in=0; }
    }
    fclose(fp);
    if (gnum==1) printf("无对局记录。\n");
}

// ---------- 棋盘与移动规则 ----------
void initBoard() {
    const char *start[8] = {"rnbqkbnr","pppppppp","........","........","........","........","PPPPPPPP","RNBQKBNR"};
    for (int r=0; r<8; r++) for (int c=0; c<8; c++) game.board[r][c] = start[r][c];
    game.whiteToMove = 1;
...
int clearPath(int sr,int sc,int dr,int dc) {
    int drs = (dr>sr)?1:(dr<sr)?-1:0, dcs = (dc>sc)?1:(dc<sc)?-1:0;
...
int canMovePawn(int sr,int sc,int dr,int dc,int white) {
    ...
    if (sc==dc && t=='.') {
        if (dr == sr+dir) return 1;
        if (sr==startRow && dr==sr+2*dir && game.board[sr+dir][sc]=='.') return 1;
    }
    if (abs(dc-sc)==1 && dr==sr+dir) {
        if (isEnemy(t,white)) return 1;
...
int canMoveKing(int sr,int sc,int dr,int dc) {
    int drs=abs(dr-sr), dcs=abs(dc-sc);
    if (drs==0 && dcs==2) return canCastle(sr,sc,dr,dc, isWhite(game.board[sr][sc]));
    return drs<=1 && dcs<=1;
}
...
int executeMove(int sr,int sc,int dr,int dc,int white,int silentPromo) {
    ...
    // 更新车移动标记
    if (tolower(piece)=='r') {
        if (white) { if (sr==7 && sc==0) game.whiteCastleQueenSide=0; if (sr==7 && sc==7) game.whiteCastleKingSide=0; }
        else       { if (sr==0 && sc==0) game.blackCastleQueenSide=0; if (sr==0 && sc==7) game.blackCastleKingSide=0; }
    }
    if (tolower(target)=='r') {
        if (target=='R') { if (dr==7 && dc==0) game.whiteCastleQueenSide=0; if (dr==7 && dc==7) game.whiteCastleKingSide=0; }
        else            { if (dr==0 && dc==0) game.blackCastleQueenSide=0; if (dr==0 && dc==7) game.blackCastleKingSide=0; }
    }
    ...
    } else game.enPassantValid = 0;

    if (isKingInCheck(white)) { game = backup; return 0; }
    ...
    if (tolower(piece)=='p' && (dr==0 || dr==7)) {
        ...
        if (!silentPromo) {
            ...
            char inp[8];
            while (1) {
                fgets(inp,8,stdin); if (inp[0]=='\n') continue;
                char c = toupper(inp[0]);
                if (c=='Q'||c=='R'||c=='N'||c=='B') { promo = white ? c : tolower(c); break; }
        ...
        if (isFriendly(game.board[sr][sc], white))
            for (int dr=0; dr<8; dr++) for (int dc=0; dc<8; dc++)
                if (canMovePiece(sr,sc,dr,dc,white)) {
                    if (executeMove(sr,sc,dr,dc,white,1)) { game = bak; return 1; }
                    game = bak;
                }
    return 0;
}
...
void printBoard() {
    ...
    for (int i=0;i<8;i++) {
        int r = order[i];
        printf("%d |", 8-r);
        for (int c=0;c<8;c++) printf(" %c |", game.board[r][c]);
        printf(" %d\n  +---+---+---+---+---+---+---+---+\n", 8-r);
    }
    printf("    a   b   c   d   e   f   g   h\n");
}
...
void replayGame(int idx) {
    ...
    if (!fp) { printf("无法打开棋谱。\n"); return; }
    char line[200];
    int cur=0, in=0, moveCnt=0;
    char moves[MAX_MOVES][10];
    char result[50]="";
    while (fgets(line, sizeof(line), fp)) {
        if (strstr(line, "[GAME_START]")) { cur++; if (cur==idx) { in=1; moveCnt=0; result[0]=0; } }
        else if (in) {
            if (strstr(line, "[RESULT]")) sscanf(line, "[RESULT] %[^\n]", result);
            else if (strstr(line, "[GAME_END]")) break;
            else {
                char mv[10];
                if (sscanf(line, "%*d. %s", mv)==1 && moveCnt<MAX_MOVES)
                    strcpy(moves[moveCnt++], mv);
            }
        }
    }
    fclose(fp);
    if (!in || moveCnt==0) { printf("未找到对局。\n"); return; }
    printf("\n回放第%d局 (结果: %s)\n按回车下一步，q退出\n", idx, result);
    GameState saved = game;
    initBoard();
    for (int i=0; i<moveCnt; i++) {
        char src[8], dst[8], promo;
        parseMoveString(moves[i], src, dst, &promo);
        int sr,sc,dr,dc;
        parseSquare(src,&sr,&sc); parseSquare(dst,&dr,&dc);
        executeMove(sr,sc,dr,dc, (i%2==0), 1);
        if (promo) game.board[dr][dc] = promo;
        printBoard();
        printf("第%d步: %s\n", i+1, moves[i]);
        if (i==moveCnt-1) break;
        char inp[10]; fgets(inp,10,stdin);
        if (inp[0]=='q' || inp[0]=='Q') { printf("中断回放。\n"); break; }
    }
    game = saved;
}
...
void continueGame() {
    ...
    while (1) {
        ...
        if (strcmp(src,"quit")==0 || strcmp(src,"exit")==0) {
            ...
            printf("游戏结束（%s）\n", res);
            if (game.moveCount) finishGame(res, 0,0,0,1);
            return;
        }
        ...
        applyView(&sr, &sc); applyView(&dr, &dc);
        ...
        if (tolower(nowPiece)!='p' && (dr==0 || dr==7)) {
            char tmp[10]; sprintf(tmp, "%s%c", moveStr, nowPiece);
            strcpy(moveStr, tmp);
        }
        saveCurrentMove(moveStr, game.moveCount+1);
        if (game.moveCount < MAX_MOVES) strcpy(game.currentGameMoves[game.moveCount], moveStr);
        game.moveCount++;
        ...
        if (isKingInCheck(!game.whiteToMove)) {
            if (!hasLegalMove(!game.whiteToMove)) {
                ...
                if (game.whiteToMove) finishGame("白方赢", 1,0,0,0);
                else finishGame("黑方赢", 0,1,0,0);
                return;
            }
            printf("将军！\n");
...
int main() {
    ...
    while (1) {
        ...
        int ch = atoi(line);
        if (!currentUser.username[0]) {
            if (ch==1) { if (login()) checkAndResumeUnfinishedGame(); }
            else if (ch==2) registerUser();
            else if (ch==3) { printf("再见！\n"); return 0; }
            else printf("无效选择。\n");
        } else {
            switch (ch) {
                ...
                case 7: listGames();
                    printf("输入回放序号 (0返回): ");
                    int idx; if (scanf("%d",&idx)==1 && idx>0) replayGame(idx);
                    while(getchar()!='\n'); break;
