...
typedef struct
{
    ...
    int enPassantValid;       // 上一回合是否走了双步兵
    int enPassantRow, enPassantCol; // 可以被吃过路兵的位置（目标格的行列）
    int moveCount;
...
void removeUnfinishedGameAtIndex(int index)
{
    ...
    if (index < 0 || index >= cnt)
        return;
    int moveCount = games[index].moveCount;
    const char *result = (moveCount % 2 == 0) ? "白方中途退出" : "黑方中途退出";
...
void continueGame()
{
    ...
    while (1)
    {
        ...
        // 注意：不要调用 applyView！棋盘显示已翻转，内部坐标始终是标准代数坐标解析结果
...
void newGame()
{
    ...
    printf("\n新游戏开始！\n");
    printInstructions();
    // 立即写入 GAME_START 标记，确保未完成对局能被正确识别
    FILE *fp = openUserFile("games", "a");
    if (fp)
    {
        fprintf(fp, "[GAME_START]\n");
