...
// 完成对局并更新统计（不自动写入 games 文件，由调用者负责写入 [RESULT]）
...
int isSameGame(UnfinishedGame *a, UnfinishedGame *b)
{
    ...
    for (int i = 0; i < a->moveCount; i++)
        if (strcmp(a->moves[i], b->moves[i]) != 0)
            return 0;
    return 1;
}

// 删除指定未完成对局（index 来自 scanUnfinishedGames 的返回数组），在原位置标记为中途退出
void removeUnfinishedGameAtIndex(int index)
{
    ...
    UnfinishedGame *target = &games[index];
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        if (strcmp(line, "[GAME_START]") == 0)
        {
            ...
            {
                // 上一个游戏如果是未完成的，且与目标相同，则这里应该被跳过（已处理）
                if (!skipThis)
                {
                    fputs("[GAME_START]\n", tmp);
        ...
        else if (strstr(line, "[GAME_END]") && inGame)
        {
            ...
            else
            {
                ...
                // 更新战绩
                finishGameStatsOnly(result, 0, 0, 0, 1);
            }
            inGame = 0;
            parseMoves = 0;
            continue;
        ...
        else if (inGame && parseMoves && line[0] >= '0' && line[0] <= '9')
        {
            ...
            if (sscanf(line, "%*d. %s", move) == 1 && curGame.moveCount < MAX_MOVES)
            {
                ...
                {
                    skipThis = 1;   // 匹配到了，后面跳过不写入
                }
            }
        }
        else if (!inGame && !parseMoves)
        {
            // 文件中的其他文本（如空行、历史对局的 [RESULT] 等）原样复制
            fputs(line, tmp);
            fputc('\n', tmp);
        }
    }
    // 如果文件末尾还有一个未完成游戏（没有 [GAME_END]）
    if (inGame && curGame.moveCount > 0)
    {
        if (!skipThis)
        {
...
int canMovePawn(int sr, int sc, int dr, int dc, int white)
{
    ...
    if (sc == dc && t == '.' && dr == sr + dir)
        return 1;
    if (sc == dc && t == '.' && sr == startRow && dr == sr + 2 * dir && game.board[sr + dir][sc] == '.')
        return 1;
    if (abs(dc - sc) == 1 && dr == sr + dir && isEnemy(t, white))
        return 1;
    if (abs(dc - sc) == 1 && dr == sr + dir && t == '.' &&
        game.enPassantValid && dr == game.enPassantRow && dc == game.enPassantCol)
        return 1;
    return 0;
}
    ...
    if (white)
    {
        ...
        if (dr == 7 && dc == 2)
        {
            if (!game.whiteCastleQueenSide)
                return 0;
            if (game.board[7][3] != '.' || game.board[7][2] != '.' || game.board[7][1] != '.')
                return 0;
    ...
    else
    {
        ...
        if (dr == 0 && dc == 2)
        {
            if (!game.blackCastleQueenSide)
                return 0;
            if (game.board[0][3] != '.' || game.board[0][2] != '.' || game.board[0][1] != '.')
                return 0;
...
int squareAttacked(int row, int col, int byWhite)
{
    for (int r = 0; r < 8; r++)
    {
        for (int c = 0; c < 8; c++)
        {
            ...
            {
            case 'p':
                ok = canPawnAttack(r, c, row, col, byWhite);
                break;
            case 'n':
                ok = canMoveKnight(r, c, row, col);
                break;
            case 'b':
                ok = canMoveBishop(r, c, row, col);
                break;
            case 'r':
                ok = canMoveRook(r, c, row, col);
                break;
            case 'q':
                ok = canMoveQueen(r, c, row, col);
                break;
            case 'k':
                ok = canMoveKing(r, c, row, col);
                break;
            }
            if (ok)
                return 1;
...
int canMovePiece(int sr, int sc, int dr, int dc, int white)
{
    ...
    if (p == '.' || !isFriendly(p, white))
        return 0;
    if (isFriendly(game.board[dr][dc], white))
        return 0;
    char low = tolower(p);
    switch (low)
    {
...
int executeMove(int sr, int sc, int dr, int dc, int white, int silentPromo)
{
    ...
    if (tolower(piece) == 'r')
    {
        ...
        {
            if (sr == 7 && sc == 0)
                game.whiteCastleQueenSide = 0;
            if (sr == 7 && sc == 7)
                game.whiteCastleKingSide = 0;
        }
        else
        {
            if (sr == 0 && sc == 0)
                game.blackCastleQueenSide = 0;
            if (sr == 0 && sc == 7)
                game.blackCastleKingSide = 0;
        }
    }
    if (tolower(target) == 'r')
    {
...
int loadGameFromMoves(char moves[][10], int moveCnt)
{
    ...
    for (int i = 0; i < moveCnt; i++)
    {
        ...
        if (!parseMoveString(moves[i], src, dst, &promo))
            return 0;
        int sr, sc, dr, dc;
        ...
        if (game.board[sr][sc] == '.')
            return 0;
        if (!executeMove(sr, sc, dr, dc, (i % 2 == 0), 1))
            return 0;
        if (promo)
            game.board[dr][dc] = promo;
    }
    ...
    while (1)
    {
        ...
        if (strcmp(src, "quit") == 0 || strcmp(src, "exit") == 0)
        {
            ...
            if (game.moveCount > 0)
            {
                ...
                finishGameStatsOnly(res, 0, 0, 0, 1);
            }
            else
            {
                // 没有走棋，删除刚才写入的 [GAME_START]（如果存在）
                char fname[64];
        ...
        char movingPiece = game.board[sr][sc];   // 移动前记录棋子类型
        ...
        // 仅当移动的棋子是兵且升变时才添加升变字母
        if (tolower(movingPiece) == 'p' && (dr == 0 || dr == 7))
        {
            char promoPiece = game.board[dr][dc];
            char tmp[10];
        ...
        if (isKingInCheck(!game.whiteToMove))
        {
            if (!hasLegalMove(!game.whiteToMove))
            {
                ...
                {
                    FILE *fp = openUserFile("games", "a");
                    if (fp) { fprintf(fp, "[RESULT] 白方赢\n[GAME_END]\n\n"); fclose(fp); }
                    finishGameStatsOnly("白方赢", 1, 0, 0, 0);
                }
                else
                {
                    FILE *fp = openUserFile("games", "a");
                    if (fp) { fprintf(fp, "[RESULT] 黑方赢\n[GAME_END]\n\n"); fclose(fp); }
                    finishGameStatsOnly("黑方赢", 0, 1, 0, 0);
                }
                return;
            }
            printf("将军！\n");
        ...
        else if (!hasLegalMove(!game.whiteToMove))
        {
            ...
            finishGameStatsOnly("平局", 0, 0, 1, 0);
            return;
        }
...
void newGame()
{
    ...
    printf("\n新游戏开始！\n");
    printInstructions();
    // 不再预先写入 [GAME_START]，由 saveCurrentMove 在第一步棋时自动写入
    continueGame();
}
