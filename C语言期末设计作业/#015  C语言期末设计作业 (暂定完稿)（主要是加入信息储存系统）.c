...
void registerUser()
{
    ...
    if (fp)
    {
        char encryptedHex[2 * MAX_PASSWORD + 1];
        encryptToHex(p1, encryptedHex);
        fprintf(fp, "%s %s\n", u, encryptedHex);
        fclose(fp);
        printf("注册成功！\n");
    }
...
int login()
{
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        char decryptedPw[MAX_PASSWORD];
        decryptFromHex(pw, decryptedPw);
        if (strcmp(un, u) == 0 && strcmp(decryptedPw, p) == 0)
        {
...
void changePassword()
{
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        {
            char encryptedHex[2 * MAX_PASSWORD + 1];
            encryptToHex(new1, encryptedHex);
            fprintf(tmp, "%s %s\n", currentUser.username, encryptedHex);
...
// 检查是否可以开始新游戏（未完成对局必须少于3个）
// 返回1表示可以开始新游戏，0表示用户取消或需要先处理
int canStartNewGame(void)
{
    ...
    while (1)
    {
        ...
        if (cnt < 3)
            return 1; // 未完成对局少于3个，可以直接开始新游戏

        printf("\n⚠️ 已有 %d 个未完成对局（最多保留3个）。必须先处理它们才能开始新游戏。\n", cnt);
        printf("请选择：\n");
        printf("  1 - 查看并处理未完成对局\n");
        printf("  2 - 放弃开始新游戏（返回主菜单）\n");
        printf("请输入：");
        char buf[10];
        fgets(buf, 10, stdin);
        int opt = atoi(buf);
        if (opt == 1)
        {
            manageUnfinishedGames(); // 进入管理界面，用户操作后返回
            // 操作完成后，继续循环检查数量
        }
        else
        {
...
int canMovePawn(int sr, int sc, int dr, int dc, int white)
{
    ...
    if (sc == dc && t == '.')
    {
        if (dr == sr + dir)
            return 1;
        if (sr == startRow && dr == sr + 2 * dir && game.board[sr + dir][sc] == '.')
            return 1;
    }
    if (abs(dc - sc) == 1 && dr == sr + dir)
    {
        if (isEnemy(t, white))
            return 1;
        if (game.enPassantValid && dr == game.enPassantRow && dc == game.enPassantCol)
            return 1;
    }
    return 0;
}
    ...
    while (1)
    {
        ...
        if (isKingInCheck(!game.whiteToMove))
        {
            if (!hasLegalMove(!game.whiteToMove))
            {
                ...
                if (game.whiteToMove)
                    finishGame("白方赢", 1, 0, 0, 0);
                else
                    finishGame("黑方赢", 0, 1, 0, 0);
                return;
            }
            printf("将军！\n");
        }
